      ...
      <div className="style.submissionWrapper">
        <div className="user-info">
          <Link to="/" className="profile-pic">
            <img src={submission.creatorId.profilePicture} style={{ width: "40px", height: "40px"}}alt="profile-pic" />
          </Link>
          <div>
            <Link to="/">
              <h2  style= {{fontSize: "19px", color: "black !important",}}>{submission.creatorId.username}</h2>
            </Link>
            <h4 style={{fontSize: "13px", fontWeight: "normal", marginTop: "3px"}}>{dateTimeConversion(submission.updatedAt)}</h4>
          </div>
        </div>

        <div className="content" style={{ whiteSpace: "pre-wrap", fontSize: "17px", marginTop: "10px" }}>
          {submission.content}
        </div>
      
      <div style={{height: "45px", display:"flex", justifyContent: "flex-end",}}>
        {totalPoints !== 0 && (
          <div style={{display: "flex"}}>
            <p style= {{marginTop: "13px", fontWeight:"bold"}}>Grade: </p>
            <div style={{ display: "inline-block", padding: "5px", marginRight: "10px"}}>
              <input
                name="name"
                style={{
                  height: "40px",
